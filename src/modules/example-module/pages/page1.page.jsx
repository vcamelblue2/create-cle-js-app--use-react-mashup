...
import { RC, RCF, RCDef } from "../../../mashup/utils/generic-rc-wrapper";
import { Router } from "cle.js/routing/lite_routing"

import { css } from "../../../utils/csz";

export const ReactComponentsPage = async (state, params)=>{
  ...
  return cle.div({
    ...
    RC(()=>
      <div>
        <a onClick={(e)=>{e.preventDefault(); Router.navigate("/example-module/tests")}}  href={"/tests"} >
          Go to Test page
        </a>
      </div>
    ),
    RC(()=>
      <div>
        <a onClick={(e)=>{e.preventDefault(); Router.navigate("/example-module/html")}}  href={"/html"} >
          Go to Html page
        </a>
      </div>
    ),
